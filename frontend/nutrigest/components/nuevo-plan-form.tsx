...
export default function NuevoPlanForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirigir al listado de planes después del registro exitoso
      router.push("/dashboard/planes")
    } catch (err) {
      console.error("Error al crear plan:", err)
      toast({
        title: "Error",
        description: "Ocurrió un error al crear el plan nutricional.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
      ...
      <CardContent className="pt-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-4">
            <Button type="button" variant="outline" onClick={() => router.push("/dashboard/planes")}>
              Cancelar
            </Button>
